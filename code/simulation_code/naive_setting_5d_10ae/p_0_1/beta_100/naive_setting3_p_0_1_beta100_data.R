...
num_drugs <- 5 #5, 15, 20, 50
num_ae <- 10 #10, 20, 50, 100
num_reports <- 300 #100, 500, 1000
num_sim <- 1000 #50
p <- 0.1 #prob of drug being given: 0.1, 0.3, 0.5
...
#define beta coefficients (all positive)
beta <- matrix(0,3,num_ae)
beta[1,1:5] <- 100 #naive value (drug1 associated with first 5 AEs)
beta[2,1:3] <- 100 #naive value (drug2 associated with first 3 AEs)
beta[3,1] <- 100 #naive value (drug3 associated with 1 AE)
...
data_path <- "/Users/reetikasarkar/Dropbox/My Mac (Reetika’s MacBook Pro)/Documents/dissertation_research/codes/data/naive_setting_5d_10ae/p_0_1/beta_100"
...
for (sim in 1:num_sim) {
  ...
  for (row in 1:num_reports) {
    for (ae in 1:num_ae) {
      x[row,ae] <- rbinom(1,1,p) #all AEs are independent
    }
    p1 <- x[row,] %*% beta[1,] 
    y[row,1] <- rbinom(1,1,(exp(p1)/(1+exp(p1))))
    p2 <- x[row,] %*% beta[2,] 
    y[row,2] <- rbinom(1,1,(exp(p2)/(1+exp(p2))))
    p3 <- x[row,] %*% beta[3,] 
    y[row,3] <- rbinom(1,1,(exp(p3)/(1+exp(p3))))
    y[row,4] <- rbinom(1,1,p)
    y[row,5] <- rbinom(1,1,p)
  }
