---
title: "naive_setting_3_p_0.1_beta100"
...
data_loc <- "/Users/reetikasarkar/Dropbox/My Mac (Reetika’s MacBook Pro)/Documents/dissertation_research/apriori_project/codes/data/naive_setting_5d_10ae/p_0_1/beta_100/sim_"

results_loc <- "/Users/reetikasarkar/Dropbox/My Mac (Reetika’s MacBook Pro)/Documents/dissertation_research/apriori_project/codes/results/naive_setting_5d_10ae/p_0_1/beta_100"
...
imp_2_set <- data.frame(rbind(c("d1", "a1"), c("d1", "a2"), c("d1", "a3"), c("d1", "a4"), c("d1", "a5"), c("d2", "a1"), c("d2", "a2"), c("d2", "a3"), c("d3", "a1")))
names(imp_2_set) <- c("item1", "item2")
# c("d1", "d2"), c("d1", "d3"), c("d2", "d3"), 
```
...
for (sim in 1:num_sim) {
  ...
  for (conf in 1:num_conf_val) {
    ...
    # 2 itemsets
      ...
      sn_conf <- ifelse((tp_conf==0 && fn_conf==0), NA, tp_conf/(tp_conf+fn_conf))
      sp_conf <- ifelse((tn_conf==0 && fp_conf==0), NA, tn_conf/(tn_conf+fp_conf))
      ppv_conf <- ifelse((tp_conf==0 && fp_conf==0), NA, tp_conf/(tp_conf+fp_conf))
      npv_conf <- ifelse((tn_conf==0 && fn_conf==0), NA, tn_conf/(tn_conf+fn_conf))
      #acc_conf <- (tp_conf+tn_conf)/(tp_conf+fp_conf+fn_conf+tn_conf)
      #y_index_conf <- ifelse((sn_conf + sp_conf - 1 < 0), 0, sn_conf + sp_conf - 1)
      fscore_conf <- ifelse(((ppv_conf==0 || is.na(ppv_conf)) && (sn_conf==0 || is.na(sn_conf))), NA, 2 * (ppv_conf * sn_conf) / (ppv_conf + sn_conf))
