...
data_path <- "/Users/reetikasarkar/Dropbox/My Mac (Reetika’s MacBook Pro)/Documents/dissertation_research/codes/data/naive_setting_5d_10ae/p_0_3/beta_0"
...
for (sim in 1:num_sim) {
  ...
  for (row in 1:num_reports) {
    ...
    #y[row,1] <- rbinom(1,1,p)
    #y[row,2] <- rbinom(1,1,p)
    #y[row,3] <- rbinom(1,1,p)
    y[row,1] <- rbinom(1,1,p)
    y[row,2] <- rbinom(1,1,p)
    y[row,3] <- rbinom(1,1,p)
